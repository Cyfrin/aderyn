...
- [High Issues](#high-issues)
  ...
  - [H-5: EnumerableSet.remove in loop corrupts the order](#h-5-enumerablesetremove-in-loop-corrupts-the-order)
  ...
  - [H-19: ETH transfered without address checks](#h-19-eth-transfered-without-address-checks)
  - [H-20: Delegatecall made to an arbitrary address](#h-20-delegatecall-made-to-an-arbitrary-address)
...
## H-5: EnumerableSet.remove in loop corrupts the order
...
If the length of a dynamic array (storage variable) is directly assigned to, it may allow access to other storage slots by tweaking it's value. This practice has been depracated in newer Solidity versions
...
The call `address(payable?).send(address)` may fail because of reasons like out-of-gas, invalid receipient address or revert from the recipient, but not revert the transaction. Therefore, the boolean returned by this function call must be checked to be `true` in order to verify that the transaction was successful.
...
## H-19: ETH transfered without address checks
...
## H-20: Delegatecall made to an arbitrary address
