...
impl IssueDetector for NestedStructInMappingDetector {
    fn detect(&mut self, context: &WorkspaceContext) -> Result<bool, Box<dyn Error>> {
        ...
        for mapping in mappings {
            if let Some(type_name) = &mapping.type_name {
                if let TypeName::Mapping(mapping_type) = type_name {
                    if let TypeName::UserDefinedTypeName(user_defined_type) =
                        &*mapping_type.value_type
                    {
                        let struct_definition_ast_node =
                            context.nodes.get(&user_defined_type.referenced_declaration);
                        if let Some(struct_definition_ast_node) = struct_definition_ast_node {
                            if let ASTNode::StructDefinition(struct_definition) =
                                struct_definition_ast_node
                            {
                                for member in struct_definition.members.iter() {
                                    if let Some(member_type_string) =
                                        &member.type_descriptions.type_string
                                    {
                                        if member_type_string.contains("struct") {
                                            ...
                                            if let Some(source_unit_ast_node) = source_unit_ast_node
                                            {
                                                let pragma_directives =
                                                    ExtractPragmaDirectives::from(
                                                        source_unit_ast_node,
                                                    )
                                                    .extracted;
