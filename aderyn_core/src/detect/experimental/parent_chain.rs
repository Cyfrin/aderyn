...
use crate::{
    ...
    capture,
    context::{
        browser::{GetAncestors, SortNodeReferencesToSequence},
...
impl IssueDetector for ParentChainDemonstrator {
    fn detect(&mut self, context: &WorkspaceContext) -> Result<bool, Box<dyn Error>> {
        for assignment in context.assignments() {
            ...
            if let Some(parent_chain) = assignment.ancestors(context) {
            ...
            if let Some(mut parent_chain) = assignment.ancestors(context) {
