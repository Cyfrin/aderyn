...
pub trait StandardInvestigatorVisitor {
    ...
    fn visit_downstream_function_definition(&self, _node: &FunctionDefinition) -> eyre::Result<()> {
        Ok(())
    }

    /// Meant to be invoked while traversing [`WorkspaceContext::reverse_callgraph`]
    fn visit_upstream_function_definition(&self, _node: &FunctionDefinition) -> eyre::Result<()> {
        Ok(())
    }

    /// Meant to be invoked while traversing [`WorkspaceContext::callgraph`]
    fn visit_downstream_modifier_definition(&self, _node: &ModifierDefinition) -> eyre::Result<()> {
        Ok(())
    }

    /// Meant to be invoked while traversing [`WorkspaceContext::reverse_callgraph`]
    fn visit_upstream_modifier_definition(&self, _node: &ModifierDefinition) -> eyre::Result<()> {
        Ok(())
    }
}
