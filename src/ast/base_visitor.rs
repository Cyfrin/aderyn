...
pub trait AstBaseVisitor {
    ...
    fn visit_node(&mut self, _node: &impl BaseNode) -> Result<bool> {
        Ok(true)
    }
    fn end_visit_node(&mut self, _node: &impl BaseNode) -> Result<()> {
        Ok(())
    }
}

pub trait BaseNode {
    fn accept(&self, _visitor: &mut impl AstBaseVisitor) -> Result<()> {
        Ok(())
    }
    // fn listAccept(
}

pub fn list_accept(list: &Vec<impl BaseNode>, visitor: &mut impl AstBaseVisitor) -> Result<()> {
    for elem in list {
        elem.accept(visitor)?;
    }
    Ok(())
