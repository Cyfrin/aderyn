...
- [NC Issues](#nc-issues)
  ...
  - [NC-11: Boolean variable name should not contain negatives.](#nc-11-boolean-variable-name-should-not-contain-negatives)
...
boolean-is-not-does-not:NC (Non Critical)
...
## NC-11: Boolean variable name should not contain negatives.

In order to avoid confusion it's better to name variables with `isXX` and `doesXX` rather than `isNotXXX` and `doesNotXXX`.

### Responsible : boolean-is-not-does-not

- Found in src/BooleanIsNot.sol [Line: 7](../../tests/contract-playground/src/BooleanIsNot.sol#L7)

	```solidity
	    bool private isNotEnjoying;
	```

- Found in src/BooleanIsNot.sol [Line: 9](../../tests/contract-playground/src/BooleanIsNot.sol#L9)

	```solidity
	    bool public doesNotDoHomework;
	```
