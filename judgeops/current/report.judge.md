...
- [Low Issues](#low-issues)
  ...
  - [L-7: Unprotected initializer](#l-7-unprotected-initializer)
- [NC Issues](#nc-issues)
  ...
  - [NC-11: Native time units would be ideal instead of custom definitions](#nc-11-native-time-units-would-be-ideal-instead-of-custom-definitions)
  - [NC-12: Inconsistency in declaring uint256/uint (or) int256/int variables within a contract](#nc-12-inconsistency-in-declaring-uint256uint-or-int256int-variables-within-a-contract)
...
time-units-not-used:NC (Non Critical)

inconsistent-type-names:NC (Non Critical)
...
- Found in src/TimeUnits.sol [Line: 2](../../tests/contract-playground/src/TimeUnits.sol#L2)
...
## NC-11: Native time units would be ideal instead of custom definitions

Suffixes like seconds, minutes, hours, days and weeks after literal numbers can be used to specify units of time where seconds are the base unit and units are considered naively.

### Responsible : time-units-not-used

- Found in src/TimeUnits.sol [Line: 7](../../tests/contract-playground/src/TimeUnits.sol#L7)

	```solidity
	    uint256 private constant DAY = 24 * 60 * 60;
	```

- Found in src/TimeUnits.sol [Line: 8](../../tests/contract-playground/src/TimeUnits.sol#L8)

	```solidity
	    uint256 private constant WEEKS = 7 * 24 * 60 * 60;
	```

- Found in src/TimeUnits.sol [Line: 9](../../tests/contract-playground/src/TimeUnits.sol#L9)

	```solidity
	    uint256 private constant MONTH = 30 * 7 * 24 * 60 * 60;
	```



## NC-12: Inconsistency in declaring uint256/uint (or) int256/int variables within a contract
